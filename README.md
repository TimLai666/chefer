...
- [~] **GUI apps on macOS** — *host side now implemented; end-to-end gated on real-Mac VZ validation.* The in-guest half (kiosk `cage` + Xwayland on a virtio-gpu scanout) is **already built and proven on the WHP path**, and the macOS host side is now written: `chefer-vz-helper --gui` adds a virtio-gpu scanout (1280×800 default, `CHEFER_VZ_GUI_SIZE=WxH` override) + USB keyboard/absolute-pointer devices and shows the VM in an AppKit window via `VZVirtualMachineView` (VZ feeds HID back automatically; closing the window ends the app, same semantics as WHP). The host↔guest **clipboard** (text + PNG, same token-authenticated wire protocol as WHP, host side on `NSPasteboard`) is implemented too, connecting directly to the guest IP over the VZ NAT. All of it compiles + signs on CI (macOS runner `swiftc`), but a GitHub runner cannot boot a VZ guest — display/input/clipboard need a physical Mac to certify. Design in [docs/DESIGN.md](docs/DESIGN.md).
