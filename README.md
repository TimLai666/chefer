...
|---|---|---|---|
| **Run the single-file app** | ✅ rootless user namespaces, zero dependencies | ✅ WSL2 — a minimal Chefer-dedicated distro is auto-provisioned for the run and best-effort removed when the app exits | 🔧 Virtualization.framework micro-VM; guest path validated on Linux+QEMU, real-Mac VZ boot pending |
...
| `chefer doctor [--kit-dir <dir>]...` | Check whether the current machine can build/run Chefer apps; prints PASS/WARN/FAIL diagnostics with actionable fixes, including the planned Windows WHP fallback host preflight. |
...
single executable ── user double-clicks ──> chefer-runtime
  ...
  └─ pick platform backend:
       ...
       Windows → WSL2 (auto-provisioned minimal distro; WHP fallback preflights host/bundle readiness but reports unavailable until implemented)
...
- **On Windows the runtime currently requires WSL2** (`wsl --install` once, if not present) — a chefer-packaged app cannot run on a Windows machine without WSL2 today. WSL2 itself runs on the **Hyper-V hypervisor** (enabled via the lightweight "Virtual Machine Platform" feature — **not** the full Hyper-V role, so it works on Windows Home — plus hardware virtualization in BIOS). `chefer doctor` can preflight the planned Windows Hypervisor Platform path, and Windows-target builds can now carry the WHP appliance/helper contract files, but running without WSL2 still requires the future WHP VM boot shim. That roadmap path would be **CPU-only** (no GPU; see above).
