...
- Inter-service networking is implicit: services in an app share one network namespace, so a service reaches another at `127.0.0.1:<port>`. Only `ports:` entries get a host→guest proxy. The app-level `network:` field (`shared` default | `internal` | `bridge`) controls host isolation — see the [Roadmap](#roadmap).
...
- **Per-app network isolation is opt-in (default is still `shared`).** With the default `network: shared`, all services in an app share one network namespace, so a service with no `ports:` is still reachable from sibling services — and on Windows, WSL2's `wslrelay` auto-mirrors any in-VM loopback port to the Windows host, so a "db with no ports" is in practice still reachable from the host. Set `network: internal` or `network: bridge` to give the app its own netns where **only declared `ports:` are reachable** and undeclared ports are walled off (validated on native Linux; see [Roadmap](#roadmap) for flipping the default and the WSL2 path). See [examples/demo/README.md](examples/demo/README.md) for a measured demonstration of the default's gap.
...
- **Host-isolated internal network + opt-in port exposure** — *available as opt-in today, not yet the default.* Set `network: internal` (own netns, only declared `ports:` bridged out, no outbound) or `network: bridge` (same + outbound NAT via a bundled `pasta`, like Docker's default) to wall an app off from the host so undeclared ports are genuinely unreachable. Validated on native Linux (amd64+arm64, rootless) in CI. Remaining:
  - **Flip the default from `shared` to `bridge`** once it has soaked — today the default stays `shared` (services share one netns; on Windows WSL2's `wslrelay` mirrors any listening port to the host), preserving existing behavior.
  - **Validate the static-`pasta` release path** — the bridge outbound E2E currently runs against a system `pasta` (`apt install passt`); `scripts/build-pasta.sh` (static musl pasta bundled into the kit) is wired into `release.yml` but not yet exercised by a real release.
  - **WSL2 / macOS-VM isolation path** — the same guest-agent netns code runs inside the distro/VM, but the host-side `wslrelay`/UDP integration for `internal`/`bridge` hasn't been specifically validated yet (native Linux is done).
- **Faster app startup** — cut first-run latency: the single file currently re-extracts its bundle to a temp dir and assembles each service's rootfs from zstd layers on launch (and, on Windows, imports the WSL distro on first run). Planned: a persistent extracted-bundle/rootfs cache keyed on content hash (skip re-extraction on subsequent runs), parallel layer decompression, and overlayfs/lazy rootfs assembly so large images (e.g. the Qt GUI demo) start in seconds.
