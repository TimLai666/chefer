...
- [~] **GPU access / CUDA compute** — *opt-in `gpu: true` binds host GPU device nodes (validated on WSL2); native-Linux NVIDIA lib injection implemented, awaiting NVIDIA-hardware validation.* Set `gpu: true` on a service and Chefer binds the host's render/compute device nodes into that service's container — on **native Linux** `/dev/nvidia*` + `/dev/dri`; on the **Windows WSL2 backend** `/dev/dxg` + a bind of the host `/usr/lib/wsl/lib` (its host-version-matched `libdxcore`/`libcuda`/… ) → CUDA/DirectML/OpenCL/Vulkan work with nothing extra in the image. The same passthrough covers ROCm/OpenCL/Vulkan compute, GL/Vulkan rendering, and VAAPI/NVENC video wherever the nodes + libs exist. **Validated on real WSL2** — with `gpu: true`, the container sees `/dev/dxg`, `/dev/dri` (`card0`/`renderD128`) and `/usr/lib/wsl/lib`, with `LD_LIBRARY_PATH` pointed at the driver libs. It is a **clear startup error** on the non-WSL Windows (WHP) micro-VM and the macOS VM — a bare WHP/VZ VM can't tap Microsoft/Apple GPU paravirtualization, so GPU compute stays **native-Linux + WSL2 only**. On *native* Linux, **host-driver-version-matched userspace lib injection is now implemented** (à la the NVIDIA Container Toolkit): Chefer reads the driver version from `/proc/driver/nvidia/version` and binds the matching `libcuda`/`libnvidia-*` (`.so.<version>` + soname `.so.1` names + `LD_LIBRARY_PATH`) into the container — version parse + lib selection are unit-tested, but **CUDA-in-container is not yet validated on real NVIDIA hardware** (gated like the macOS-VZ items on hardware this repo's dev machine lacks). No path to Apple NPU / CoreML from a Linux guest.
