...
| **GUI services** | ✅ X11 / Wayland socket passthrough | ✅ WSL2 via WSLg ([gui-demo](examples/gui-demo)); **✅ WHP (no WSL)** — bundle-embedded `cage` compositor on a virtio-gpu scanout shown in a native window, with keyboard/mouse + bidirectional text clipboard (software-rendered; validated on real hardware) | 🔧 (same guest path, real-Mac VZ pending) |
...
- **GUI support**: native Linux passes through X11/Wayland sockets; Windows WSL2 relies on WSLg; the **Windows WHP path (no WSL) runs a bundle-embedded `cage` Wayland compositor** (with Xwayland for X11 apps) on a virtio-gpu scanout, shown in a native window with keyboard/mouse and bidirectional text clipboard. GUI is always **software-rendered** (llvmpipe, no GPU acceleration — see below).
