...
- **Host-isolated networking — shipped; `bridge` is now the default.** Omitting `network:` gives an app its own netns where **only declared `ports:` are reachable and undeclared ports are walled off from the host**, with outbound NAT via a bundled `pasta` (like Docker's default). `network: internal` is the same without outbound; `network: shared` opts back into the old "everything shares one netns / undeclared ports reachable" behavior. Validated on native Linux (amd64+arm64, rootless) in CI and on real WSL2 (`internal` isolation). Remaining polish:
  - **macOS-VM isolation path** — the same guest-agent netns code runs inside the VM; pending real-Mac VZ validation (the macOS backend itself is pending hardware, below).
  - **`bridge` outbound on WSL2** — outbound NAT needs `pasta` reachable inside the distro; the bundled `pasta` may need copying into the distro (drvfs exec), so `bridge` on Windows can currently fall back to `internal` (no outbound). `internal` isolation on WSL2 is validated.
