...
- **Windows**（`wsl2`）：可用性 = `wsl.exe --status` 成功且支援 WSL2。執行：
  ...
  7. **免 WSL 的 `whp` 後端**：以 **Windows Hypervisor Platform（WHP）** 開機 bundle 內附的 Linux micro-VM appliance（與 macOS `vz` 共用同一 kernel/initramfs/guest-agent），作為 `wsl2` 的替代後端，移除對 WSL2 的依賴（仍需硬體虛擬化 + WHP 功能）。對完全無虛擬化的機器，另可選擇性 bundle 軟體模擬（QEMU/TCG）作為最終備援（可跑但慢）。backend 抽象（`ExecBackend`）已納入 `whp`，Windows 後端排序為 `wsl2` → `whp`；`whp` 以 `WinHvPlatform.dll` + `WHvGetCapability(HypervisorPresent)` 做 host preflight，並在有 bundle context 時檢查 `vm/chefer-vmlinuz-<arch>`、`vm/chefer-initramfs-<arch>`、`agents/chefer-whp-helper-<arch>.exe` 是否存在——三者皆備且 hypervisor 可用時回傳 `Available`，由 runtime 的 `run()` spawn helper 執行完整開機，stdout 逐行解析 `CHEFER_GUEST_IP=<ipv4>` / `CHEFER_GUEST_EXIT=<code>` 標記。helper CLI contract：`chefer-whp-helper --kernel <p> --initramfs <p> --cmdline <s> --bundle-dir <p> --data-dir <p> --cpus <n> --memory-mib <n> [--timeout <secs>]`。
    ...
    **完整開機模式（boot shim）**（已實作並在 Windows 上實測通過——Alpine virt 6.6 kernel 完整開機到 init）：
    ...
    8. **virtio-mmio 裝置模型（規劃；目標＝讓真 bundle 在無 WSL 的 Windows 一鍵跑起來）**：目前 boot shim 只能開機跑 initramfs 內的最小 init，**無任何把 bundle 傳進 VM 的途徑、也無網路**（`EXIT_MEM_ACCESS` 一律報錯，§4 第 3 點）。要跑真 workload，需在 boot shim 內補出與 vz 等效（vz 靠 VZ 內建 virtiofs/virtio-net/virtio-console）的裝置模型。選 **virtio-over-MMIO** 而非 virtio-pci：WHP 不模擬 PCI host bridge，且 x86 上 virtio-mmio 可純靠 kernel cmdline `virtio_mmio.device=<size>@<base>:<irq>` 靜態註冊，免 PCI 列舉與 device tree。契約：
       ...
       - **分階段里程碑（每個可獨立驗證）**：M1 transport（emulator 接線 + virtqueue 純邏輯 + dummy device 完成 feature 協商）→ M2 virtio-blk(bundle ro)（guest `mount /dev/vda` 讀到 bundle，serial 確認）→ M3 virtio-blk(data rw) + 關機回寫 → M4 virtio-net + 埠轉發（`CHEFER_GUEST_IP`、host→guest TCP 通）→ M5 appliance 自適應 init + kernel config + build → M6 端到端真 bundle（實機）。
...
| 單檔執行（linux/amd64,arm64 服務）| ✅ namespaces | ✅ WSL2；✅ whp（boot shim 實測通過，vmm-backend 已接線） | 🔜 vz 骨架（明確錯誤）|
