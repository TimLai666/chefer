...
- 埠代理：
  ...
  - **macOS（vz）**：runtime 的 `start_port_proxies` **整個略過**（TCP+UDP、含 host≠guest）——vz 的 guest 埠不會出現在 host localhost（VZ NAT 無 wslrelay 等自動轉送），host→guest 一律由 vz 後端取得 guest IP 後 relay `127.0.0.1:host → <vm_ip>:guest`（實體 Mac 實測：runtime 在此綁 `127.0.0.1:host` 只會與 vz 後端的 relay 搶同一顆埠）。
...
- **macOS**（`vz`，Apple Virtualization.framework）：macOS 沒有現成 Linux，必須自行開一台輕量 Linux micro-VM 來跑容器。設計如下（**狀態：已於實體 Apple Silicon（macOS 26，2026-07）完整驗證通過**——開機/exit code/TCP+UDP 埠轉發/bridge 出網+DNS/多服務/persist/GUI 顯示+HID+剪貼簿+動態解析度全綠；`availability()` 依真實前提（host arch、內嵌 appliance、內嵌 helper、macOS 13+）決定可用性、**無實驗性 env 鎖**（macOS 上 vz 是唯一後端）。Intel（x86_64）走完全相同的程式路徑、appliance 換 arch，但實機 VZ 開機僅在 Apple Silicon 驗過（GHA runner 兩種 arch 都開不了 VZ guest）。）
  ...
  - **可用性判定（誠實回報，無實驗性 env 鎖）**：**實機驗證已於 2026-07 在 Apple Silicon（macOS 26）完成**（開機/exit code/TCP+UDP 埠轉發/bridge 出網+DNS/多服務/persist/GUI 顯示+HID+剪貼簿+動態解析度），故不再上 `CHEFER_VZ_EXPERIMENTAL` 這道 env 鎖（macOS 上 vz 是唯一執行後端，鎖著等於雙擊預設不能跑，違背單檔精神）。`availability()` 改為純依真實前提回報：① host arch 支援（x86_64/aarch64）② bundle 內嵌 appliance（`vm/chefer-vmlinuz-<arch>`＋`-initramfs`）③ bundle 內嵌 helper（`agents/chefer-vz-helper-<arch>`，或 `CHEFER_VZ_HELPER` 指向本機自建）④ macOS ≥13（virtiofs）——缺任一即 `Unavailable` 附可行動訊息。**Intel（x86_64）取捨**：走完全相同的 Rust／Swift 路徑、只換 appliance arch，但實機 VZ 開機僅在 Apple Silicon 驗過（CI 兩種 arch 都開不了 VZ guest）；此差異以文件註記，不另上 runtime 鎖（否則會擋掉很可能可行的路徑）。
  ...
  - **WHP GUI（M8，規劃）— host 側自建顯示與輸入（guest 側與 vz 完全共用上述 overlay/cage 路線）**：vz 有 `VZVirtualMachineView` 免費送顯示+HID，WHP 全要自己來。**2D only**（不做 virgl/GPU 加速——WHP 路線本就 CPU-only，cage 用 llvmpipe 軟算繪；host 只做 framebuffer 搬運與顯示）；指標採 **virtio-input tablet（絕對座標）** 避免滑鼠捕捉問題。
    ...
    - **M8-c host 視窗**（分兩步）：
      - ✅ **M8-c-1 視窗模組 + 跨執行緒橋接（本機驗證達成）**：`virtio/gui_bridge.rs`（跨平台純邏輯：`SharedFrame`＝VM loop 寫、視窗執行緒讀的 BGRA framebuffer 拷貝 + generation；`SharedInput`＝視窗寫、VM loop 讀的 evdev 事件佇列；`to_bgra` 像素格式轉換；`vk_to_evdev` Win32 VK→Linux keycode；11 單元測試）＋ `gui_window.rs`（`#[cfg(windows)]` Win32 訊息迴圈：`StretchDIBits` blit、`WM_MOUSEMOVE`→tablet 絕對座標、`WM_KEYDOWN`→keyboard、關窗→`closed_by_user`、`request_close` 反向關窗）。**鎖粒度**：只共享一塊 framebuffer 拷貝 + 一條事件佇列，不鎖整個裝置。**驗證**：`--gui-selftest` 在本機（無 WHP）開視窗畫動畫測試圖並印輸入事件——實機確認視窗建立、GDI blit、`WM_MOUSEMOVE`→`abs_from_window`→`SharedInput` 全鏈可用。
      - ✅ **M8-c-2 接線（compile-check 完成，待實機 M8-d）**：三個新 virtio-mmio 裝置接進 boot loop。**IRQ 選 PIC1 空閒線（gpu=1、keyboard=3、tablet=4）而非 8+**：IRQ 8-15 在 PIC2/slave，本 helper 的 `deliver_pending_pic_irq` 只遞送 PIC1；用 PIC1 空閒線即沿用 blk/net/data 完全相同的單裝置遞送路徑（IRQ1 因 i8042 init 失敗、serial IRQ3/4 在此 8250 不觸發而空閒）。bases：gpu `0xD000_0600`、keyboard `0xD000_0800`、tablet `0xD000_0A00`。gpu `service()` 處理 control+cursor queue、`RESOURCE_FLUSH` 後 `SharedFrame::update`；input eventq 由 VM loop 每輪 `GuiDevices::pump`（drain `SharedInput`→分流 keyboard/tablet→填 eventq）、statusq 收掉。`--gui` 旗標（vmm-backend `whp.rs` 依 manifest 有無 gui 服務帶入）才掛這三個裝置 + cmdline + 開視窗，否則 headless 完全不變。使用者關窗→VM loop 偵測 `closed_by_user`→乾淨收 app；guest 先結束→`GuiHandle::join` 反向關窗。全 compile-check（Windows 原生 + musl 交叉）+ 單元測試。
    - ✅ **M8-d 實機端到端達成（真 Windows + WHP）**：`interface_mode: gui` 的 xclock demo 在實體 Windows 桌面上顯示於「Chefer」視窗——WHP micro-VM 開機 → virtio-gpu 初始化（`/dev/dri/card0`）→ cage compositor → xclock 經 llvmpipe 算繪到 virtio-gpu scanout → 我方 gpu 裝置收 `RESOURCE_FLUSH` → `SharedFrame` → Win32 `StretchDIBits` blit → 桌面可見（截圖驗證時鐘畫面）。**實機才抓到的關鍵 bug**：virtio-gpu driver 查詢 host-visible SHM region（virtio-mmio SHM 暫存器 0xAC–0xBC），我方 `Mmio` 原本回 0 → kernel 誤以為存在一塊 region 而 reserve 失敗、probe `-EBUSY`（`Could not reserve host visible region`）→ 無 `/dev/dri`、cage `Found 0 GPUs`。修法：SHMLen/Base 依 virtio spec 回全 1（len=-1=region 不存在），driver 便跳過 host-visible 路徑走傳統 2D。**視窗解析度**：✅ **EDID 達成 1280×800**——早期 cage/wlroots 無 EDID → connector 無偏好模式 → 挑 640×480。gpu 裝置現宣告 `VIRTIO_GPU_F_EDID` 並實作 `GET_EDID`（回一份以 `width×height`@60 為第一（＝偏好）detailed timing 的 128-byte EDID 1.4 block，checksum 有效；`build_edid` 見 `virtio/gpu.rs`），guest DRM connector 據此建出 1280×800 偏好模式、cage 採用。host 視窗另有 **resize-to-content** 保底（`WM_TIMER` 偵測 flush 尺寸變化 → `SetWindowPos` 跟著 guest scanout 1:1），任何解析度都不失真。實機驗證：xclock 於「Chefer」視窗滿版 1280×800、時鐘正圓（PrintWindow 擷取）。鍵鼠輸入端到端與剪貼簿列 M8-e。
...
- **原生 Linux — AMD（ROCm）/ Intel（oneAPI）**：與 NVIDIA 不同，**不需 host 庫注入**——ROCm/HIP、Intel oneAPI/Level-Zero/OpenCL 的 userspace 由容器映像自帶（`rocm/*`、`intel/oneapi` 等），kernel ABI 較穩定。chefer 只需綁裝置節點：**AMD** = `/dev/kfd`（KFD 計算裝置）+ `/dev/dri`；**Intel** = `/dev/dri`（render node）。即標準容器化 ROCm 模型（`--device=/dev/kfd --device=/dev/dri`）。**已實作（`/dev/kfd` 加入節點探測、含單元測試），但因無 AMD/Intel 硬體尚未實機驗證**（gated on hardware，同 macOS VZ 慣例）。
...
**節點探測與綁定**（`guest-agent/src/gpu.rs`）：探測 host 的 `/dev/dxg`、`/dev/dri`、`/dev/kfd`（AMD ROCm）、`/dev/nvidia*`（`nvidia0…`/`nvidiactl`/`nvidia-uvm`/`nvidia-uvm-tools`/`nvidia-modeset`/`nvidia-caps`）、`/usr/lib/wsl/lib`（存在時連同兄弟目錄 `/usr/lib/wsl/drivers` 一併綁，見上）；存在者以既有 `BindEntry` 機制綁進容器（裝置節點可讀寫、lib/drivers 目錄唯讀），在 `setup_dev` 建好 `/dev` tmpfs 之後套用（節點落進該 tmpfs）。**只綁存在者**（沿用 host 基本裝置「不存在略過」的慣例）。rootless 原生 Linux 綁 root 擁有的 `/dev/nvidia*` 受節點權限限制（多為 0666 可用；失敗則透傳 mount 錯誤）。
...
- **ML 框架（PyTorch）實測通過**：`pytorch/pytorch:2.4.1-cuda12.1-cudnn9-runtime` + `gpu: true`——`torch.cuda.is_available()==True`、偵測到 RTX 4070（cap 8.9）、GPU matmul（cuBLAS）誤差 ~5.7e-5、真訓練迴圈（forward+autograd+SGD）loss 1.12→0.72、`memory_allocated≈20MB`、`PYTORCH_GPU_PASS`、exit 0。映像自帶 `libcudart`/cuDNN/cuBLAS，chefer 只注入 driver（`libcuda`+`/dev/nvidia*`）。
...
- **視訊（NVENC/NVDEC）實測通過**：`libnvidia-encode`/`libnvcuvid` 走 dlopen、不需圖形環境——容器內 `ffmpeg -c:v h264_nvenc` 編碼 60 幀成功、exit 0（stock `ubuntu` image + apt 裝 ffmpeg）。
- **OpenGL / Vulkan 算繪：無頭容器不支援（非 chefer 限制）**：`libGLX_nvidia`（NVIDIA 的 Vulkan ICD）會拉進 `libnvidia-glcore`，後者參照 Xorg 的 `ErrorF` 與 glibc 2.34 已移除的 `__malloc_hook`，需**實際圖形/X server 環境**才能初始化。實測即使補注入 vendor manifest（`nvidia_icd.json`/`10_nvidia.json`），Vulkan 仍掛在 `Could not get vkCreateInstance for ICD libGLX_nvidia.so.0` 並退回 `llvmpipe`——且 **`docker --gpus all` + `NVIDIA_DRIVER_CAPABILITIES=all`（nvidia-container-toolkit 參考實作）於同一無頭 host 亦相同失敗**，證明是 host 驅動對圖形環境的硬需求，非容器檔案注入能解。故 GL/Vulkan 庫僅為完整性對齊；**算繪只在具圖形/顯示環境的 host 才可行**（chefer 的 GUI 路徑另走 X11/Wayland socket 直通 + 軟算繪，見 §6）。因此不注入 ICD manifest（無頭下只會多噴 loader error 且無實益）。
...
- 子命令：
  ...
  - `doctor [--kit-dir <dir>]...`：檢查目前 host 是否具備 build/run Chefer app 的基本條件；輸出每項 PASS/WARN/FAIL 與英文可行動建議。檢查 OS/arch、平台後端前提（Windows WSL2 + 規劃中的 WHP fallback host preflight、Linux root 或 unprivileged user namespaces）、kit 探索（沿用 `chefer_bundle::kit`，含 guest-agent/pasta/appliance/vz helper/whp helper/gui overlay——後者缺少僅 WARN，因為只有打包 Windows/macOS 的 GUI app 才需要）、Chefer 版本/build 資訊，以及平台預設 data 目錄是否可寫。任一 FAIL → exit code 非 0。
  - `inspect <single-file>`：讀 footer + 串流掃描 payload tar（不執行、不落地解壓），顯示 manifest 摘要、app network/console、payload 大小、**「Packed by chefer」**（取自 manifest 的 `app.builder_version`，舊 bundle 無此欄位則顯示 unknown）、每個 service 的 platform/layers/各層壓縮大小/persist/ports/mounts/interface/depends_on/healthcheck/gpu，以及內嵌 `agents/`、`vm/` 協力檔清單。
